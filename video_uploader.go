...
func uploadOneVideo(manager *TaskManager, video string, supp *Supp) error {
	...
	if err != nil {
		...
		err = manager.RecordMessage(supp, groupMsg, "video_placeholder", "photo", "", groupMsg.Caption, "", thumbnail, "", 0, 1)
		if err != nil {
			log.Println(err)
		}
	}
	...
	if manager != nil {
		err = manager.RecordMessage(supp, videoMsg, "video", "video", "", videoMsg.Caption, "", video, "", 0, 1)
		if err != nil {
			log.Println(err)
		}
		manager.IncrementUploadedVideo(supp)
	}
...
func uploadVideos(manager *TaskManager, t *qbit.Torrent, supp *Supp) error {
	path := t.ContentPath
	var videos []string
	...
	if manager != nil {
		manager.AddMediaTotals(supp, len(videos), -1)
	}
	var errGroup error
	for idx, video := range videos {
		if manager != nil {
			manager.UpdateTaskProgress(supp, string(TaskUploadingVideo), fmt.Sprintf("上传视频 %d/%d: %s", idx+1, len(videos), filepath.Base(video)), "")
		}
		err := uploadOneVideo(manager, video, supp)
		if err != nil {
			log.Println(err)
